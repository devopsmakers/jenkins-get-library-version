...
class PipelineTestHelper extends BasePipelineTest {
    ...
    void registerScriptedMethods() {
        ...
        helper.registerAllowedMethod('readYaml', [Map.class], { args -> readYaml(args) })
        helper.registerAllowedMethod('readYaml', [String.class], { args -> readYaml(args) })
    ...
    Object readJSON(def args) {
        def jsonSlurper = new JsonSlurper()
        if (args.text) {
            return jsonSlurper.parseText(args.text)
        }
        return null
    }
    ...
    Object readYaml(def args) {
        ...
        String yamleFileContent = new File(file).getText('UTF-8')
    ...
    /**
     * Tests for a files existence
     */
    Bool fileExists(def args) {
        String file = null
        if (args instanceof String || args instanceof GString) {
            file = args
        } else if (args instanceof Map) {
